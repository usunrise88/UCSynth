...
## Статус — этапы 0–1 и 3 на железе ✅, этап 2 (GUI) + этапы 4–5 (модуляция, эффекты) собраны
...
**Этапы 4–5 — модуляция и эффекты** (чистый DSP, host-тестируемый; собраны в облаке, замер CPU/PSRAM на железе — за пользователем):

- **Модуляция (4)** — 2 глобальных LFO (sine/tri/saw/square/S&H); **wave-огибающая** (8 точек, зацикливаемая) →
  морф wavetable (кроссфейд форм); гибкая **мод-матрица** 8 слотов (источник → приёмник × глубина), роутинг из GUI.
- **Эффекты (5)**, стерео-тракт после суммы голосов — **overdrive** (tanh-шейпер) → **delay** (кольцо в PSRAM,
  feedback + damp, стерео-ширина) → **reverb** (Freeverb, 8 гребёнок + 4 allpass/канал, буферы в PSRAM).
- Реестр вырос 34 → 86; всё видно в пульте (панели LFO/Wave-огиб./Матрица/Overdrive/Delay/Reverb). Ревер —
  потолок CPU: на плотном патче с полной цепочкой возможен упор (тогда — лестница урезаний, см. `tech-debt.md`).

Дальше — этап 6 (пресеты). Полный план — [`docs/roadmap.md`](docs/roadmap.md).
