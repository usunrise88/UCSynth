...
void wavetable_init(float sample_rate)
{
    ...
    for (int w = 0; w < WAVE_COUNT; ++w) {
        const int w_off = off;        // начало всех таблиц этой формы в пуле (нормируем их вместе)
        float wave_max = 0.0f;        // наибольший пик среди mip формы
        for (int m = 0; m < MIP_COUNT; ++m) {
            ...
            if (maxabs > wave_max) wave_max = maxabs;
            t[L] = t[0];                                          // guard = первый семпл
        }

        // Нормировка ОДНИМ коэффициентом на всю форму, а не отдельно на каждый mip.
        //
        // Раньше каждая таблица нормировалась на свой пик, а на верхних mip остаётся 1–4 гармоники:
        // пик усечённого ряда отличается от полного, поэтому нормировка съезжала по основному тону.
        // Замер амплитуды 1-й гармоники SAW до правки: mip 0 → 0.553 (0 дБ), mip 7 → 0.684
        // (+1.85 дБ), mip 9/10 → 0.950 (+4.69 дБ). Границы mip лежат на 1280/2560/5120/10240 Гц,
        // так что портаменто пилой C6→C7 давало ступеньку посреди глайда, pitch-LFO через границу —
        // паразитную AM на частоте LFO, а тумблер lofi (принудительный mip 0) на высокой ноте —
        // скачок до 4.7 дБ. Комментарий «Гиббса не критичен» описывал другой масштаб явления.
        //
        // Делим на НАИБОЛЬШИЙ пик среди mip формы, а не на пик mip 0: у меандра верхние mip имеют
        // пик ВЫШЕ полного спектра (одна гармоника даёт 4/π от плато, тогда как полный ряд
        // нормирован по выбросу Гиббса ≈1.09 от плато — итого 1.105). Нормировка по mip 0 сломала бы
        // гарантию [-1,1], на которую опирается интерполяция и весь тракт ниже.
        const float norm = (wave_max > 1e-9f) ? (1.0f / wave_max) : 1.0f;
        for (int i = w_off; i < off; ++i) s_pool[i] *= norm;      // включая guard-семплы
    }
}
