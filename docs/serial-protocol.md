...
LIST:           55 AA 01 03 <crc>
  ...
  ← ... PARAM #4..#33 (голос 3.1–3.6: ADSR, осц, фильтр, lo-fi, полифония, glide — см. control.h) ...
  ← LISTEND:    55 AA 03 83 2200 <crc>                        (count = 34)
...
Скрипт делает LIST → STAT → выключает `test_tone` → играет ноты и прогоняет демо голоса
(3.1 ADSR/latch, 3.2 фильтр/резонанс/VCF-свип, 3.3 детюн/шум/ring, 3.4 lo-fi). Ожидаемо: список
из **34** параметров (реестр строится динамически — новые параметры GUI/скрипт подхватывают через
LIST без правок), STAT показывает heap/uptime + `cpu_permille` (‰ бюджета аудио-блока) и
`underruns`, ноты меняют высоту, высокие ноты звучат чисто (band-limit), демо слышно по секциям.
Строки `ESP_LOG` в потоке — норма, скрипт их пропускает (не проходят CRC).
